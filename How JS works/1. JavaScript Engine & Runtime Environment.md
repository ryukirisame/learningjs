...
6. Some time later... the browser gets the HTTP response:
   ...
   - ```cpp
      ...
      ```
   - The browser **directly calls** the JS engine's `resolvePromise()` function, which immediately updates the Promise state, stores the result, and schedules callbacks in the microtask queue. (Remember, JS engine itself is written in C++, that's why browser is able to tell the JS engine to resolve the promise.)
