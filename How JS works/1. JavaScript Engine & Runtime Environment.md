...
async function fetchData() {
  console.log('2');
  await null; // Equivalent to await Promise.resolve(null)
  console.log('3');
}
...
| Call stack now empty → event loop runs microtask → resumes `fetchData()` |                                                |
| prints `3`                                                               |                                                |

### How does the function gets "paused" and "resumed" - What happens under the hood
...
```
is **conceptually** transformed by the engine into:
...
```
- Notice: The code after await becomes a callback passed to `.then()`. That's why its scheduled as a microtask - because `.then()` always is.
...
```
is _functionally_ similar to this:
```js
somePromise
  .then(() => {
    console.log('after await');
