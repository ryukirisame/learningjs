...
2. JS calls `fetch()` (Synchronous):
   ...
   - Browser (fetch implementation written in C++) immediately returns a Promise (status: "pending"). The executor function of the Promise is provided by the browser's fetch API, so we don't need to worry about the executor function of the promise. 
...
6. Some time later... the browser gets the HTTP response:
   ...
   - ```cpp
      ...
      ```
   - The resolution essentially make the browser tell the JS engine DIRECTLY to update the state of the promise, store the result and schedule the callbacks in the microtask.
