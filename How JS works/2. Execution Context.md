...
const calculator = (function() {
    ...
    // Return public API - methods defined inline
    return {
        add: function(num) {
            if (validate(num)) {
                result += num;
...
#### Revealing Module Pattern (this is actually the name of the pattern)
```js
const calculator = (function() {
    // Private variables
    let result = 0;
    
    // All functions defined the same way (private by default)
    function validate(num) {
        return typeof num === 'number';
    }
    ...
    // Reveal which functions are public
    return {
        add: add,           // or just: add,
        subtract: subtract, // or just: subtract,
        getResult: getResult,
        reset: reset
    };
})();
...
- The classic module pattern returns an object where you define public methods inline in the return statement.
- The revealing module pattern defines all functions (both public and private) as regular named functions, then reveals which ones are public by referencing them in the return object. The return statement "reveals" the public interface
