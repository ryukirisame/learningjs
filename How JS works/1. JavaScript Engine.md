...
- When a Promise resolves, the engine queues the `.then()` callback into the microtask queue. The browser doesn't directly enqueue it, but if the Promise was created by a browser API (like fetch), the browser triggers the resolution, which then causes the engine to enqueue the callback.
### 2. queueMicrotask(...)
...
### 3. setTimeout(...)
- Who handles it? The host (browser)
- Where does the callback go? Macrotask queue
- Explanation:
   - JS engine sees setTimeout(fn, 1000) and says:
      - “Hey browser, please set a timer for 1 second.”
   - Browser starts the timer.
   - After 1 second, the browser says:
      - “Okay, time’s up! Add this fn to the macrotask queue!”
...
### 5. fetch(...)
- Who handles it? The browser
- Where does the callback go? First: resolved promise → microtask
...
- The browser does the network request, not JS.
- When response comes back, the browser resolves the Promise.
- Resolving the Promise causes the engine to queue the `.then(...)` callback as a microtask.

So the browser triggers the resolution, but the JS engine handles `.then` as a microtask.


## Microtasks & Promises 

Here's what happens when we use `new Promise(...)`
```js
...
3. `resolve("done")` is called
   - the engine schedules the `.then()` callback into the microtask queue
4. Once the current task ends, the event loop runs the `.then()` callback from the microtask queue
   - prints `"Resolved with: done"`
     
All of this — including calling the executor, managing internal state, scheduling .then() — is handled by the JS engine.


- When you write `new Promise((resolve, reject) => { ... })`, the executor function runs immediately, synchronously, inside the JS engine.
- Inside that executor:
   - If you call `resolve(...)` or `reject(...)`, the engine marks the promise as **fulfilled** or **rejected**
   - While the Promise is still pending, JS engine needs to hold onto the callback until the Promise resolves or rejects. The callbacks of `then` and `catch` are stored in the Promise object itself. And where does the Promise object resides? The heap!
...
We can think of Promise object like this:
```js
Promise {
  [[Status]]: "pending",
  [[Reactions]]: [ callback1, callback2, ... ]
}
```
- When resolve() or reject() is called: The engine moves those callbacks from the Promise into the microtask queue
...
```
1. `new Promise(...)` → engine creates the Promise
2. `.then(...)` → callback saved inside the Promise, not executed yet
3. `setTimeout` runs 1 sec later → `resolve('ok')`
4. Engine sees Promise status changes → moves `.then()` callback to microtask queue
5. After current task → event loop runs the callback
...
2. JS calls `fetch()`:
   ...
   - fetch immediately returns a Promise (handled by JS engine)
   - `.then(...)` is attached to that Promise
3. JS engine runs `console.log('3')` → prints `3`
4. Some time later... the browser gets the HTTP response:
   - It resolves the Promise
   - This notifies the JS engine
   - The engine schedules the `.then()` callback in the microtask queue
5. Once the call stack is empty, event loop runs the `.then()` → prints `2`
